<template>
	<view>
		...
		<view class="cu-list">
			<transition-group class="transition-group grid col-3" name="list-fade" tag="uni-view">
				<view class="cu-item" v-for="(item,index) in cuIcon" :key="index" v-if="item.isShow">
					<view class="padding-tb">
						<text class="lg block" :class="'cuIcon-' + item.name"></text>
						<text>{{ item.name }}</text>
					</view>
				</view>
			</transition-group>
...
<script>
	export default {
		...
		methods: {
			searchIcon (e) {
				clearTimeout(this.oIsSearch);
				this.oIsSearch = setTimeout(() => {
					let key = e.detail.value.toLowerCase();
					this.cuIcon.forEach(item => item.isShow = item.name.includes(key))
				}, 100)
			}
		}
	}
